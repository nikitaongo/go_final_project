...
5. Реализован поиск задач с учётом регистра, а также по дате. Дата может быть указана в формате `дд.мм.гггг`, в качестве разделителей могут использоваться пробел и символы: `.-/_`
...
```
2. Запустить контейнер с пробросом портов:
```bash   
docker run --rm -p 7540:7540 go-final
```
3. Перейти в браузере по ссылке: `http://localhost:7540`(или на другой порт, если изменён)

- Сборка образа происходит под архитектуру хоста.
- Локальные переменные указаны явно в dockerfile и могут быть изменены.
